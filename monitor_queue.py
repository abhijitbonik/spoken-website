...
if reaches >= MONGO_BULK_INSERT_COUNT items, pop the leftmost MONGO_BULK_INSERT_COUNT items
...
while (True):
    ...
    if REDIS_CLIENT.llen('tasks') >= settings.MONGO_BULK_INSERT_COUNT:
        
        try:

            logs = REDIS_CLIENT.lrange('tasks', 0, settings.MONGO_BULK_INSERT_COUNT - 1)
            ...
            REDIS_CLIENT.ltrim('tasks', start=settings.MONGO_BULK_INSERT_COUNT, end=-1)
