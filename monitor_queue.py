...
if reaches >= 1000 items, pop the leftmost 1000 items
...
while (True):
    ...
    if REDIS_CLIENT.llen('tasks') >= 1000:
        
        try:

            logs = REDIS_CLIENT.lrange('tasks', 0, 999)

            # trim the queue to remove the leftmost 1000 logs
            ...
            REDIS_CLIENT.ltrim('tasks', start=1000, end=REDIS_CLIENT.llen('tasks'))
